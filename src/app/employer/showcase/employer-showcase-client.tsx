...
export function EmployerShowcaseClient() {
          ...
          {visibleProjects.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {visibleProjects.map((project) => (
                <Card
                  key={project.id}
                  className="overflow-hidden transition hover:border-primary/50 hover:bg-surface-raised"
                >
                  <Link
                    className="block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    href={`/employer/projects/${encodeURIComponent(project.id)}`}
                  >
                    <ProjectPreview
