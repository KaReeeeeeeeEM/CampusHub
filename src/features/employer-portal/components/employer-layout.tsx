...
import { EmployerSidebar } from "@/features/employer-portal/components/employer-sidebar";
import { EmployerTopNavigation } from "@/features/employer-portal/components/employer-top-navigation";
...
export function EmployerLayout({ children, user }: EmployerLayoutProps) {
    ...
    <div className="dashboard-shell min-h-screen text-foreground">
      ...
      <div className={cn("lg:p-2", sidebarCollapsed ? "lg:pl-16" : "lg:pl-64")}>
        <div className="dashboard-app-frame">
          <EmployerTopNavigation user={user} />
          <main className="min-h-0 flex-1 overflow-y-auto">
            <DashboardPageTransition>{children}</DashboardPageTransition>
          </main>
          <DashboardIntroTour
            role="Employer"
            storageKey="campushub:intro:employer-dashboard"
          />
        </div>
      </div>
    </div>
